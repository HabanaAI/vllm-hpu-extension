...
def generate_decode_buckets(bs_bucket_config, blocks_bucket_config,
    ...
    if os.environ.get('VLLM_DECODE_BLOCK_BUCKET_MAX') is None and os.environ.get(
                'VLLM_CONTIGUOUS_PA', 'true').lower() == 'true':
        blocks_bucket_config[2] = max_blocks
    block_buckets = warmup_range(blocks_bucket_config)
    if os.environ.get(
            'VLLM_CONTIGUOUS_PA', 'true').lower() == 'true' and os.environ.get('VLLM_DECODE_BLOCK_BUCKET_MAX') is None and max_blocks not in block_buckets:
