...
def fix_cache_inputs(json_data, args):
    ...
    for layer_index in range(len(layer_indexes)):
        ...
        matmul_av_input = json_data['Nodes'].get(matmul_av_key, {}).get(
            'inputs', [None, None])[1]
        v_cache_input = json_data['Nodes'].get(v_cache_key,
                                               {}).get('inputs', [None])[0]
        matmul_qk_input = json_data['Nodes'].get(matmul_qk_key, {}).get(
            'inputs', [None, None])[1]
        k_cache_input = json_data['Nodes'].get(k_cache_key,
                                               {}).get('inputs', [None])[0]
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "full path to the directory of the measurements that should be fixed")
    parser.add_argument(
        "-o",
        "--out",
        type=str,
        default=os.getcwd(),
        help=
        "path to the directory where the fixed measurements will be written",
    )
    parser.add_argument(
        "-d",
        "--deepseek",
...
def main(args):
    ...
    measurements_paths_ranges = [
        ...
        if measurement_path.endswith(".json") and 'MAXABS_HW' not in
        measurement_path and "mod_list" not in measurement_path
    ]
    measurements_paths_scales = [
        measurement_path for measurement_path in measurements_paths
        if measurement_path.endswith(".json") and 'MAXABS_HW' in
        measurement_path and "mod_list" not in measurement_path
    ]
